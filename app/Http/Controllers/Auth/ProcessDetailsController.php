...
class ProcessDetailsController extends Controller
{
    ...
    public function show(Request $request)
    {
        ...
        $informacionPermisoUsuario = $this->consultarPermisosUsuarioInteresados($numeroDocumento, $tipoDocumento);
        ...
        $informacionActuaciones = $this->consultarInformacionActuaciones($uuidProcesoDisciplinario);
        ...
        $datosRequestPortalLog['detalle'] = "El usuario con el identificador N° $portalUserId ha visto el detalle del proceso $uuidProcesoDisciplinario";
     ...
     * con el uuid
     */
    public function consultarPermisosUsuarioInteresados($numeroDocumento, $tipoDocumento)
    {
     ...
     * con el uuid
     */
    public function consultarInformacionActuaciones($uuid)
    {
        // Se realiza la consulta a la tabla
        $results = DB::select("SELECT A.UUID, TO_CHAR(A.CREATED_AT, 'DD/MM/YYYY') AS CREATED_AT, MA.NOMBRE_ACTUACION, A.AUTO, A.DOCUMENTO_RUTA
                               ...
                               AND A.ID_ESTADO_ACTUACION IN (1, 5)
                               AND MA.VISIBLE = 1");
